...
int snmpGet(MIB *thismib, struct messageStruct *response, int *len )
{
	...
	switch(thismib->dataType) {
		...
		case COUNTER64 :
			{
				*(response->buffer+response->index+1) = LONG_SIZE;  /* Length of Counter64 is 8 bytes */
				h2nll_byte((uint64_t)thismib->u.intval64, response->buffer+response->index+2);
				compactInt(response->buffer+response->index);
				*len = (int) *(response->buffer+response->index+1);
			}
			break;
	...
	switch(dataType) {
		...
		case COUNTER64 :
			if (thismib->set != NULL) {
				...
				thismib->dataLen = LONG_SIZE;  /* Length of Counter64 is 8 bytes */
				thismib->u.intval64 = *((uint64_t *)val);
			}
		default:
			return INVALID_DATA_TYPE;
	}
...
int gethostaddr( char *hostname, struct sockaddr_in *sin )
{
	...
	if (hostname == NULL) {
		sin->sin_addr.s_addr = inet_addr(hostIpAddr);
	}
	else {
		if ( (he=gethostbyname(hostname)) == NULL ) {
			if (debug)
				printf("Unable to resolve hostname %s during initialisation (%d).\n", hostname, h_errno);
			return FAIL;
		}
...
int initSnmpAgent( int port, char *entoid, char *rocommstr, char *rwcommstr )
{
	...
	mibTree = miblistnew(0);
  	gethostaddr( NULL, &servaddr );
  	inet_ntop(AF_INET, &(servaddr.sin_addr), hostIpAddr, 16);
	if (debug)
...
int processSNMP( void )
{
	...
	if (request.len > 0) {
		...
		if (response.len > 0) {
			...
			sendto(snmpfd, response.buffer, response.index, 0, (struct sockaddr *)&from, fromlen);
			if (debug) {
			...
			if (debug) {
				printf("Parse fail! Error %d at byte position %d. Error Status %u, Index %u\n", response.len, request.index, errorStatus, errorIndex);
			}
		return response.len;
	}
	else return FAIL;
...
size_t snmp_encode_oid(const uint32_t *oid, uint8_t oid_len, uint8_t *buf, size_t buf_len)
{
    ...
    for (int i = 2; i < oid_len; i++)
    {
        ...
        /* Encode value in reverse */
        encoded[enc_len++] = value & 0x7F;
        value >>= 7;

        while (value > 0)
        {
            encoded[enc_len++] = (value & 0x7F) | 0x80;
            value >>= 7;
        }

        /* Write in correct order */
        if (pos + enc_len > buf_len)
            return 0;

        for (int j = enc_len - 1; j >= 0; j--)
        {
            buf[pos++] = encoded[j];
        }
    }
...
void print_hex_dump(const char *tag, const uint8_t *data, int len)
{
    char buf[256] = {0};
    int pos = 0;
    for (int i = 0; i < len; i++)
