...
static esp_err_t screen_patch_handler(httpd_req_t *req)
{
    ...
    char *debug_screen = cJSON_Print(screen_root);

    ESP_LOGI(TAG, "Stored screen = %s", debug_screen);
        ...
        for (int i = 0; i < sz; i++)
        {
            ...
            if (path_obj && path_obj->valuestring && val_obj)
            {
                ...
                while (token != NULL)
                {
                    ...
                    {
                        // Found leaf key
                        cJSON *existing = cJSON_GetObjectItem(curr, last_token);
                        if (!existing)
                        {
                            ESP_LOGW(TAG, "PATCH path not found: %s",
                        ...
                        if (!cJSON_ReplaceItemInObject(
                                curr,
                                last_token,
                                cJSON_Duplicate(val_obj, true)))
                        {
                            ESP_LOGW(TAG, "Failed to replace: %s",
                                    path_obj->valuestring);
                            rejected++;
                            break;
                        }
