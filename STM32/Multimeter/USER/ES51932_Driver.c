...
void GPIO_Configuration(void) {
    ...
    // Configure PB3, PB4, PB5, PB8, PB9, PB10, PB13, PB14, PB15 as output push-pull
    GPIO_InitStructure.GPIO_Pin = GPIO_Pin_3 | GPIO_Pin_4 | GPIO_Pin_5 | GPIO_Pin_8 | GPIO_Pin_9 | GPIO_Pin_10 | GPIO_Pin_13 | GPIO_Pin_14 | GPIO_Pin_15;
    GPIO_InitStructure.GPIO_Speed = GPIO_Speed_50MHz;
...
static void ControlSW1(uint16_t sw1) {
    if (sw1 == 1) {
        GPIO_ResetBits(GPIOB, GPIO_Pin_14); // Turn on SW1 (low)
    } else {
        GPIO_SetBits(GPIOB, GPIO_Pin_14); // Turn off SW1 (high)
    }
}
...
void ES51932_Init(void) {
    ...
    // Keep the control task enabled; measurement reception runs in USART3_IRQHandler.
    xTaskCreate(ControlChannelsTask, ( signed portCHAR * )"ControlChannelsTask", configMINIMAL_STACK_SIZE, NULL, tskIDLE_PRIORITY+3, NULL);
...
void SetMode(uint8_t mode) {
    ...
    ControlSW1(mode == MODE_AC_VOLTAGE_MEASUREMENT);
