...
#define SDA_LOW()  IIC_SDA_LO();//(GPIOA->BSRR = 0x40000000) // set SDA to low
#define SDA_OPEN() IIC_SDA_HI();(GPIOB->BSRR = 0x00004000) // set SDA to open-drain
#define SDA_READ   READ_SDA//(GPIOB->IDR  & 0x4000)     // read SDA

// SCL on port B, bit 13               /* -- adapt the defines for your uC -- */
#define SCL_LOW()  IIC_SCL_LO();//(GPIOB->BSRR = 0x20000000) // set SCL to low
#define SCL_OPEN() IIC_SCL_HI();//(GPIOB->BSRR = 0x00002000) // set SCL to open-drain
...
//  
//  IIC_SDA_HI();// IIC_SDA_HI();                  // I2C-bus idle mode SDA released
//  IIC_SCL_HI();                  // I2C-bus idle mode SCL released
//  
//  // SDA on port B, bit 14
...
//==============================================================================
  ...
  {
    if((mask & txByte) == 0) IIC_SDA_LO(); // masking txByte, write bit to SDA-Line
    else                     IIC_SDA_HI();
    delay_us(5 );               // data set-up time (t_SU;DAT)
    IIC_SCL_HI();                         // generate clock pulse on SCL
    delay_us(5 );               // SCL high time (t_HIGH)
    IIC_SCL_LO();;
    delay_us(1 );               // data hold time(t_HD;DAT)
  }
	error = IIC_Wait_Ack();
//  IIC_SDA_HI();                           // release SDA-line
//	SDA_IN();
//	delay_us(1);
//  IIC_SCL_HI();                           // clk #9 for ack
//  delay_us(5);                 // data set-up time (t_SU;DAT)
//	
...
//==============================================================================
  ...
  for(mask = 0x80; mask > 0; mask >>= 1) // shift bit for masking (8 times)
  { 
    IIC_SCL_HI();                          // start clock on SCL-line
	...
	SDA_OUT();
  if(ack == ACK) IIC_SDA_LO();              // send acknowledge if necessary
  else           IIC_SDA_HI();
