...
uint8_t count_hold_on_bip_to_mstp;  // ��yabe����T3000������ڷ���ʱ����Ҫ��д������豸
...
//	U8_T invokeid_mstp;
	...
	{	
		...
		else if(protocal == BAC_IP_CLIENT)
		{
			...
			else if((object_type == BAC_AO + 1) || ((object_type == OUT + 1) ))
			{
			...
			else if((object_type == BAC_BO + 1) || ((object_type == OUT + 1) ))
			{
...
//   decode ptransfer
	...
	if(command > 100)   // write command
	{
		...
		{		
			ChangeFlash = 3; // write it into flash now
		} 
		else
		{
		...
		// TBD: add more write command
			...
			switch(command)
			{
				...
				break;
				case WRITE_PVAR:
					if(private_header.point_end_instance < MAX_TEMCOVARS)
						ptr = (uint8_t *)&pvars[private_header.point_start_instance];	
				break;
				default:
					break;	
...
//						update_extio_to_database();
					...
					{ 
						...
						// after udpate, need intial monitor again
						monitor_init();
						//Rtc_Set(Rtc.Clk.year,Rtc.Clk.mon,Rtc.Clk.day,Rtc.Clk.hour,Rtc.Clk.min,Rtc.Clk.sec,0);
/*						//RTC_Get();
