...
EXT_RAM_BSS_ATTR STR_SEND_BUF mstp_bac_buf[10];

U8_T rec_mstp_index1; // response packets form
U8_T send_mstp_index1;
EXT_RAM_BSS_ATTR STR_SEND_BUF mstp_bac_buf1[10];*/
...
/*EXT_RAM_BSS_ATTR*/ UN_Time  update_dyndns_time;
...
EXT_RAM_BSS_ATTR Str_Extio_point  extio_points[MAX_EXTIO];
...
#else
EXT_RAM_BSS_ATTR 	Str_in_point  			inputs[MAX_INS];// _at_ 0x20000;
EXT_RAM_BSS_ATTR 	Str_out_point 			outputs[MAX_OUTS];//_at_ 0x22000;
EXT_RAM_BSS_ATTR 	Str_variable_point		vars[MAX_VARS];// _at_ 0x18000;
#endif

Str_TemcoVar_point pvars[MAX_TEMCOVARS];
...
EXT_RAM_BSS_ATTR S8_T   							var_unit[MAX_VAR_UNIT][VAR_UNIT_SIZE];


EXT_RAM_BSS_ATTR Str_Special  						Write_Special;
EXT_RAM_BSS_ATTR Str_MISC   						MISC_Info;
EXT_RAM_BSS_ATTR Str_Remote_TstDB             		Remote_tst_db;// _at_ 0x10000;
EXT_RAM_BSS_ATTR Str_Panel_Info 		 			Panel_Info;
EXT_RAM_BSS_ATTR Str_Setting_Info     				Setting_Info;
...
EXT_RAM_BSS_ATTR Mon_Data 			 				*Graphi_data;
EXT_RAM_BSS_ATTR S8_T 								Garphi_data_buf[sizeof(Mon_Data)];//  _at_ 0x41100;
EXT_RAM_BSS_ATTR Alarm_point 						alarms[MAX_ALARMS];
EXT_RAM_BSS_ATTR U8_T 			    				ind_alarms;
EXT_RAM_BSS_ATTR Alarm_set_point 	    			alarms_set[MAX_ALARMS_SET];
U8_T 			    							ind_alarms_set;
U16_T                							alarm_id;
S8_T                         					new_alarm_flag;
EXT_RAM_BSS_ATTR Units_element		    			digi_units[MAX_DIG_UNIT];
U8_T 					 						ind_passwords;
EXT_RAM_BSS_ATTR Password_point						passwords[ MAX_PASSW ];
EXT_RAM_BSS_ATTR Str_program_point	    			programs[MAX_PRGS];// _at_ 0x24000;
EXT_RAM_BSS_ATTR Str_controller_point 				controllers[MAX_CONS];// _at_ 0x25000;
EXT_RAM_BSS_ATTR Str_totalizer_point          		totalizers[MAX_TOTALIZERS];// _at_ 0x12500;
EXT_RAM_BSS_ATTR multiple_struct 					msv_data[MAX_MSV][STR_MSV_MULTIPLE_COUNT];

EXT_RAM_BSS_ATTR Str_monitor_point					monitors[MAX_MONITORS];// _at_ 0x12800;
...
EXT_RAM_BSS_ATTR Control_group_point  	 		control_groups[MAX_GRPS];// _at_ 0x14000;
//Str_grp_element		    far	    	group_data[MAX_ELEMENTS];// _at_ 0x14500;
//EXT_RAM_BSS_ATTR Str_grp_element			   	  group_data[MAX_ELEMENTS];
EXT_RAM_BSS_ATTR Str_grp_element_new   		group_data_new;

S16_T 					far							 total_elements;
S16_T 					far							 group_data_length;


EXT_RAM_BSS_ATTR Str_weekly_routine_point 	far		weekly_routines[MAX_WR];//_at_ 0x28000; // _at_ 0x15200;
EXT_RAM_BSS_ATTR Wr_one_day					far		wr_times[MAX_WR][MAX_SCHEDULES_PER_WEEK];//_at_ 0x29000 ;//_at_ 0x16000;
EXT_RAM_BSS_ATTR U8_T  wr_time_on_off[MAX_WR][MAX_SCHEDULES_PER_WEEK][8];
EXT_RAM_BSS_ATTR Str_annual_routine_point	far	 	annual_routines[MAX_AR];// _at_ 0x2a000;//_at_ 0x16500;
EXT_RAM_BSS_ATTR U8_T                   		     ar_dates[MAX_AR][AR_DATES_SIZE];//_at_ 0x2b000 ;//_at_ 0x17000;
...
EXT_RAM_BSS_ATTR NETWORK_POINTS      		 network_points_list[MAXNETWORKPOINTS];	 /* points wanted by others */
Byte              			  number_of_network_points_bacnet;
Byte              			  number_of_network_points_modbus;

EXT_RAM_BSS_ATTR REMOTE_POINTS		   		  remote_points_list[MAXREMOTEPOINTS];
Byte              			  number_of_remote_points_bacnet;
Byte              			  number_of_remote_points_modbus;

EXT_RAM_BSS_ATTR POINTS_HEADER 		local_points_list[MAXLOCALPOINTS];
Byte				 	number_of_local_points;
...
EXT_RAM_BSS_ATTR STR_REMOTE_PANEL_DB  remote_panel_db[MAX_REMOTE_PANEL_NUMBER];
	...
	for( i = 0; i < MAX_INFO_TYPE; i++, inf++ )
	{
		switch(i)
		{
			...
			case AR_DATA:               // 17 ar_dates[MAX_AR][AR_DATES_SIZE];
				...
				inf->max_points = MAX_AR;
				break;
			case TEMCOVAR:             // 17 ar_dates[MAX_AR][AR_DATES_SIZE];
				inf->address = (S8_T *)pvars;
				inf->size = sizeof(Str_TemcoVar_point);
				inf->max_points = MAX_TEMCOVARS;
				break;
			default:
				break;	
		}
...
//	memset(panelname,0,20);
	...
	memset(pvars,0,MAX_TEMCOVARS*sizeof(Str_TemcoVar_point));
}
...
//	Setting_Info.reg.backlight = Modbus.backlight;
	...
	if (Setting_Info.reg.bbmd_port == 0) {
		Setting_Info.reg.bbmd_port = 47808;
	}
	if (Setting_Info.reg.bbmd_ttl == 0) {
...
//	Setting_Info.reg.network_ID[2] = Modbus.network_ID[2];	
	...
	Setting_Info.reg.write_flash = Modbus.write_flash;
}
...
void Send_whois_to_mstp(uint32_t instance)
{
	...
	Modbus.com_config[2] == 1/*BACNET_SLAVE*/ || Modbus.com_config[0] == 1/*BACNET_SLAVE*/)
	{
...
S8_T get_point_info_by_instacne(Point_Net * point)
{
	...
	{  // R&W modbus points in program code.
		if((Modbus.com_config[0] == 7/*MODBUS_MASTER*/) || (Modbus.com_config[2] == 7/*MODBUS_MASTER*/))
		{
			// dont change com config
		}
		else
		{
			if(Modbus.com_config[0] == 2/*MODBUS_SLAVE*/)
			{
			...
			if(Modbus.com_config[2] == 2/*MODBUS_SLAVE*/)
			{
				if(Modbus.fix_com_config == 0)
				{// if com config can be changed automatically
			...
			if((Modbus.com_config[0] == 0/*NOUSE*/) && (Modbus.com_config[2] == 0/*NOUSE*/))
			{
				if(Modbus.fix_com_config == 0)
				{
//					if(count_modbus_slave[0] == 0)
...
{
#if 1
	if(Modbus.com_config[0] == 1/*BACNET_SLAVE*/ || Modbus.com_config[0] == 9/*BACNET_MASTER*/)
	{
	...
	if(Modbus.com_config[2] == 1/*BACNET_SLAVE*/ || Modbus.com_config[2] == 9/*BACNET_MASTER*/)
	{	
...
void check_mstp_packet(uint8_t * uart_rsv, uint16_t len, uint8_t port)
{
	...
	for(i = 0;i < len;i++)
	{
		if(i < len - 8)
		{
			if(uart_rsv[i] == 0x55 && uart_rsv[i + 1] == 0xff)
			{
				uint8_t ret;
				ret = CRC8(&uart_rsv[i]);
				if(ret == 0)
				{
					flag_mstp_err[port] = 0;
					Test[35]++;
					return ;
				}
				...
				)
  		{
				...
				case AMON:
					*val_ptr = monitors[point->number].status ? 1000L : 0;
					break;
				case TEMCOVAR:
			 		ptr.ptvar = &pvars[point->number];
					if((!ptr.ptvar->digital_analog) && (ptr.ptvar->range != UNUSED))
					{
						if(( ptr.ptvar->range >= ON_OFF  && ptr.ptvar->range <= HIGH_LOW )
							||(ptr.ptvar->range >= custom_digital1 // customer digital unit
							&& ptr.ptvar->range <= custom_digital8
							&& digi_units[ptr.ptvar->range - custom_digital1].direct == 1))
						{	// inverse
							*val_ptr = (ptr.ptvar->control ? 0L : 1000L);
						}
						else
						{
							*val_ptr = (ptr.ptvar->control ? 1000L : 0L);
						}
					}
					else
					{
						*val_ptr = ptr.ptvar->value;
					}
					break;
				default:
					break;
...
//					clear_dead_master();
			...
			case AMON:
				...
				break;
			case TEMCOVAR:
				sptr.ptvar = &pvars[point->number];
				if(sptr.ptvar->digital_analog == 0)
				{
					sptr.ptvar->control = *val_ptr ? 1 : 0;
				}
				temp = *val_ptr;
				sptr.ptvar->value = (temp);				
				break;
		...
		else  // network points
		{
			...
			if(index < MAXNETWORKPOINTS)
			{
...
#endif
				{
					...
					if(mode == 0)
						network_points_list[index].time_to_live = (NP_TIME_TO_LIVE * number_of_network_points_bacnet) > 100 ? 100 : (NP_TIME_TO_LIVE * number_of_network_points_bacnet);
					else
						network_points_list[index].time_to_live = PERMANENT_LIVE;
...
void init_new_analog_block( int mon_number, Str_monitor_point *mon_ptr/*, Monitor_Block *block_ptr*/ )
{
	...
	if(sample_time == 0)  // avoid overrun
		sample_time = 1;
    ...
    uart_write_bytes(0, (const char *)debug_array, strlen(debug_array));
	 ...
	 + ((uint32_t)temp[1] << 16) + ((uint32_t)temp[0] << 24);
		...
		if(get_net_point_value( ptr.pnet, &value,0,0 ) == 1)  // get rid of default value 0x55555555
		{
			...
			// only for test
			/*if(j == 0)
				write_mon_point_buf_to_flash[flash_trendlog_seg].value =  flash_trendlog_seg * 1000;
			else if(j == 1)
				write_mon_point_buf_to_flash[flash_trendlog_seg].value =  flash_trendlog_num[i * 2] * 1000;*/
