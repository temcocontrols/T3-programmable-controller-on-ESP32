...
//				return input_raw[io_index];
		...
		// find output index by AOx
		{
				...
				ptr = put_io_buf(OUT,io_index);
				if(io_index >= max_dos + max_aos + max_dos_2)
				{
					if(priority == 15)
...
void write_Output_Relinguish(uint8_t type,uint8_t i,float value)
{
	...
	/* NG3: BO/AO bacnet index may map to non-contiguous OUT (DO after AO) */
	if(max_dos_2)
	{
		if(type == BO)
		{
			if(!Get_index_by_BOx(i, &out_index))
				return;
		}
		else if(type == AO)
		{
			if(!Get_index_by_AOx(i, &out_index))
				return;
		}
	}

	ptr = put_io_buf(OUT, out_index);
	if(type == BO)
	{
		if(out_index < max_dos || (max_dos_2 && out_index >= max_dos + max_aos && out_index < max_dos + max_aos + max_dos_2))
		{
			output_relinquish[out_index] = value;
			if(ptr.pout->digital_analog == 0)
			{				
				if(( ptr.pout->range >= ON_OFF && ptr.pout->range <= HIGH_LOW )
					||(ptr.pout->range >= custom_digital1 // customer digital unit
					...
					&& digi_units[ptr.pout->range - custom_digital1].direct == 1))
				{// inverse
					output_relinquish[out_index] = value ? 0 : 1;
					ptr.pout->control = Binary_Output_Present_Value(out_index) ? 0 : 1;
				}
				else
				{
					output_relinquish[out_index] = value ? 1 : 0;
					ptr.pout->control = Binary_Output_Present_Value(out_index) ? 1 : 0;
				}

				if(ptr.pout->control)
					set_output_raw(out_index,1000);
				else
					set_output_raw(out_index,0);
			}
			else
				output_relinquish[out_index] = value;

			check_output_priority_array(out_index,0);
		}
	}
	else if(type == AO)
	{
...
float Get_Output_Relinguish(uint8_t type,uint8_t i)
{
	...
	if(max_dos_2)
	{
		if(type == BO)
		{
			if(!Get_index_by_BOx(i, &out_index))
				return 0;
			return (float)output_relinquish[out_index];
		}
		else
		{
			if(!Get_index_by_AOx(i, &out_index))
