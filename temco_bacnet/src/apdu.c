...
static confirmed_function far Confirmed_Function[MAX_BACNET_CONFIRMED_SERVICE];
...
static confirmed_function far Unrecognized_Service_Handler;
...
static unconfirmed_function	 far
...
void apdu_handler(
    BACNET_ADDRESS * src,
    uint8_t * apdu,     /* APDU data */
    uint16_t apdu_len, uint8_t protocal)
{
	BACNET_CONFIRMED_SERVICE_DATA far service_data = { 0 };
    BACNET_CONFIRMED_SERVICE_ACK_DATA far service_ack_data = { 0 };
    uint8_t far invoke_id = 0;
    uint8_t far service_choice = 0;
    uint8_t far *service_request = NULL;
    uint16_t far service_request_len = 0;
    int far len = 0;        /* counts where we are in PDU */
    uint8_t far tag_number = 0;
    uint32_t far len_value = 0;
    uint32_t far error_code = 0;
                        ...
                        service_request_len/*, src, &service_data*/);
							...
							if (service_choice == SERVICE_CONFIRMED_READ_PROPERTY) {								
										...
										handler_write_property(service_request,
												service_request_len, src, &service_data,protocal);
	            }else if (service_choice == SERVICE_CONFIRMED_WRITE_PROP_MULTIPLE) {
						...
						count_hold_on_bip_to_mstp = 10;									
								...
								|| service_choice == SERVICE_CONFIRMED_REINITIALIZE_DEVICE)
							{
								handler_device_communication_control(
									service_request,service_request_len, src, &service_data, protocal);
							}								
#endif				
...
#if 0//BAC_FILE
							else if(service_choice == SERVICE_CONFIRMED_ATOMIC_READ_FILE)
							{
								handler_atomic_read_file(service_request,service_request_len, src,&service_data,protocal);
							}
							else if(service_choice == SERVICE_CONFIRMED_ATOMIC_WRITE_FILE)
							{
...
//									char i;
							...
							if (service_choice < MAX_BACNET_UNCONFIRMED_SERVICE) {
									...
									if (service_choice == SERVICE_UNCONFIRMED_WHO_IS) 
									{
										...
										{	 
											if((apdu_len == 5) &&
												(service_request[0] == 0xdf)&& 
												(service_request[1] == 0xfd))  
											{
												flag_suspend_mstp = 1;
...
//                            (BACNET_ERROR_CODE) error_code*/);	  // tbd: chelsea
		...
		memset(&Handler_Transmit_Buffer[protocal],0,MAX_PDU);
