    ...
    BACNET_READ_ACCESS_DATA * read_access_data)
{
    ...
    BACNET_READ_ACCESS_DATA far *rpm_object;        /* current object */
    uint8_t far apdu_temp[16];      /* temp for data before copy */
    BACNET_PROPERTY_REFERENCE far *rpm_property;    /* current property */
...
{
    unsigned  far len = 0;
    uint16_t far  type = 0;  /* for decoding */
...
{
    unsigned far  len = 0;
    unsigned  far tag_len = 0;
    uint8_t  far tag_number = 0;
    uint32_t  far len_value_type = 0;
    uint32_t  far property = 0;      /* for decoding */
    uint32_t  far array_value = 0;   /* for decoding */
