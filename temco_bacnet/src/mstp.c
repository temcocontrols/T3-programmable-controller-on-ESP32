...
#else
static void printf_receive_data(
    const char *format,
    ...)
{
    format = format;
...
#else
static  void printf_receive_error(
    const char *format,
    ...)
{
    format = format;
...
uint16_t MSTP_Create_Frame(
    ...
    uint8_t * dat,     /* any data to be sent - may be null */
    uint16_t data_len)
{       /* number of bytes of data (up to 501) */
    ...
    index = 8;
    while (data_len && dat && (index < buffer_len)) {
        buffer[index] = *dat;
...
{       /* number of bytes of data (up to 501) */
    ...
    len =
        ...
        mstp_port->OutputBufferSize, frame_type, destination, source, dat,
        data_len);
    ...
    volatile struct mstp_port_struct_t *mstp_port)
{
    ...
    if ((receive_state != MSTP_RECEIVE_STATE_IDLE) &&
        (mstp_port->receive_state == MSTP_RECEIVE_STATE_IDLE)) {
        printf_receive_data("\n");
      //  fflush(stderr);    changed by chelsea
    }
    return;
}
