...
#if ASIX
Address_Cache_Entry far Address_Cache[MAX_ADDRESS_CACHE];
#endif
...
void address_remove_device(
    uint32_t device_id)
{
    Address_Cache_Entry far *pMatch;
...
static  Address_Cache_Entry *address_remove_oldest(
    void)
{
    Address_Cache_Entry far *pMatch;
    Address_Cache_Entry far *pCandidate;
    uint32_t far ulTime;
...
//        pMatch++;
//    }
		memset(Address_Cache,0,sizeof(Address_Cache_Entry) * MAX_ADDRESS_CACHE);
    ...
    BACNET_READ_RANGE_DATA * pRequest)
{
    int iLen = 0;
    int32_t iTemp = 0;
    Address_Cache_Entry *pMatch = NULL;
    BACNET_OCTET_STRING MAC_Address;
