...
//	inx = in_aux;
	...
	{	
		...
		{
#ifdef MANUAL_JUMPER			
			input_type[point] = ptr.pin->decom >> 4;
#endif


			if((ptr.pin->range >= table1) && (ptr.pin->range <= table5))
			{
				if(ptr.pin->decom >> 4 == 0)
			...
			else 
			{
				...
				else
				{
					...
					else if((ptr.pin->range == V0_10_IN) || (ptr.pin->range == P0_100_0_10V))   // 0-10v
					{
						input_type[point] = INPUT_0_10V;
					}
					else if((ptr.pin->range == I0_20ma) || (ptr.pin->range == P0_100_4_20ma))
					{
						input_type[point] = INPUT_I0_20ma;
					}
					else if((ptr.pin->range == PT1000_200_300DegC) || (ptr.pin->range == PT1000_200_570DegF)) // 0-_5v
					{
			...
			{
				ptr.pin->sub_id = 0;
				ptr.pin->sub_product = 0;
				ptr.pin->sub_number = 0;
				...
				{ 			
					...
					{				
						...
						{						
							...
							{
								ptr.pin->control = (sample > (512 * shift) ) ? 1 : 0;
							}
							else
							{
								ptr.pin->control = (sample < (512 * shift) ) ? 0 : 1;					
							}
							
							if( ptr.pin->range >= custom_digital1 && ptr.pin->range <= custom_digital8 )
							{
								ptr.pin->control = (sample < (512 * shift) ) ? 0 : 1;	
							}
							
							ptr.pin->value = swap_double(ptr.pin->control ? 1000L : 0);
								
							sample = ptr.pin->control ? 1000L : 0;
						}
...
//								break;
							...
							case table5:
								...
								sample = 1000l * test_match_custom((int)ptr.pin->range, (int)sample);	
								//sample = 1000l * sample;
								break;
							case N0_2_32counts:
							case HI_spd_count:	
						...
						//	if( ptr.pin->calibration_increment ) 
							{
						...
						ptr.pin->value = swap_double(sample);
					...
					else  // not_used_input
					{
						// if range is 0, show raw value
							...
							if(point < get_max_internal_input())
								ptr.pin->value = swap_double((U32_T)get_input_raw(point) / shift * 3000 / 1023);
							else

								ptr.pin->value = swap_double((U32_T)get_input_raw(point));
					}	
				}
				else if(ptr.pin->auto_manual == 1)	// manual
				{		
					temp = ptr.pin->decom;
					temp &= 0xf0;
					temp |= IN_NORMAL;
