...
{
    esp_err_t ret;
    /* IDF5: i2c_cmd_link_create() allocates each cmd on heap — OK for long payloads.
     * Do NOT use a small create_static buffer with i2c_master_write of 80+ bytes. */
    i2c_cmd_handle_t cmd = i2c_cmd_link_create();
    ...
    i2c_master_start(cmd);
    ret = i2c_master_write_byte(cmd, (0x74 << 1) | WRITE_BIT, ACK_CHECK_EN);
    if(ret != ESP_OK)
        goto out;
    ...
    if(count > 0)
    {
        ret = i2c_master_write(cmd, (uint8_t *)value, count, ACK_CHECK_EN);
        if(ret != ESP_OK)
            goto out;
    }
    ret = i2c_master_stop(cmd);
...
int32_t stm_i2c_read(int16_t reg, uint8_t *value,uint16_t len)
{
    ...
    uint16_t i;
    uint8_t send_ack;
    /* Default i2c_cmd_link_create() is too small for ~120 per-byte reads.
     * Keep the same write + repeated-start + read sequence that worked for 114. */
    static uint8_t link_buf[I2C_LINK_RECOMMENDED_SIZE(140)];
    i2c_cmd_handle_t cmd;

    if(value == NULL || len == 0)
        return ESP_ERR_INVALID_ARG;

    cmd = i2c_cmd_link_create_static(link_buf, sizeof(link_buf));
    if(cmd == NULL)
        return ESP_ERR_NO_MEM;
    ...
    for(i = 0; i < len; i++)
    {
        send_ack = (i < (len - 1)) ? 1 : 0; /* last byte must be NACK'ed */
        i2c_master_read_byte(cmd, &value[i], send_ack ? ACK_VAL : NACK_VAL);
    }
...
static void bmv080_data_ready(bmv080_output_t output, void* callback_parameters) {
    ...
    //    printf("PM1.0: %.1f μg/m3 | PM2.5: %.1f μg/m3 | PM10: %.1f μg/m3\n",
    //           output.pm1_mass_concentration,
     //          output.pm2_5_mass_concentration,
     //          output.pm10_mass_concentration);
    } else {
     //   if (output.is_obstructed) printf("[WARN]  传感器光学路径被遮挡\n");
     //   if (output.is_outside_measurement_range) printf("[WARN]  PM2.5 超量程（>1000 μg/m3）\n");
    }
