...
static void app_event_cb(const ChipDeviceEvent *event, intptr_t arg)
{
    ...
    case chip::DeviceLayer::DeviceEventType::kInterfaceIpAddressChanged:
        debug_info("Interface IP Address changed");
        break;
    ...
    case chip::DeviceLayer::DeviceEventType::kFailSafeTimerExpired:
        debug_info("Commissioning failed, fail safe timer expired");
        break;

    case chip::DeviceLayer::DeviceEventType::kCommissioningSessionStarted:
        debug_info("Commissioning session started");
        {
            char debug_buf[64];
            sprintf(debug_buf, "Free heap: %lu", esp_get_free_heap_size());
            debug_info(debug_buf);
            sprintf(debug_buf, "Min free heap: %lu", esp_get_minimum_free_heap_size());
            debug_info(debug_buf);
            sprintf(debug_buf, "Free internal heap: %u", heap_caps_get_free_size(MALLOC_CAP_INTERNAL));
            debug_info(debug_buf);
            sprintf(debug_buf, "Free PSRAM: %u", heap_caps_get_free_size(MALLOC_CAP_SPIRAM));
            debug_info(debug_buf);
        }
        break;

    case chip::DeviceLayer::DeviceEventType::kCommissioningSessionStopped:
        debug_info("Commissioning session stopped");
        break;
    ...
    case chip::DeviceLayer::DeviceEventType::kCommissioningWindowClosed:
        debug_info("Commissioning window closed");
        break;

    case chip::DeviceLayer::DeviceEventType::kFabricRemoved: {
        debug_info("Fabric removed successfully");
        if (chip::Server::GetInstance().GetFabricTable().FabricCount() == 0) {
            ...
            if (!commissionMgr.IsCommissioningWindowOpen()) {
                ...
                if (err != CHIP_NO_ERROR) {
                    debug_info("Failed to open commissioning window");
                }
            }
        }
        break;
    ...
    case chip::DeviceLayer::DeviceEventType::kFabricWillBeRemoved:
        debug_info("Fabric will be removed");
        break;

    case chip::DeviceLayer::DeviceEventType::kFabricUpdated:
        debug_info("Fabric is updated");
        break;

    case chip::DeviceLayer::DeviceEventType::kFabricCommitted:
        debug_info("Fabric is committed");
        break;

    case chip::DeviceLayer::DeviceEventType::kBLEDeinitialized:
        debug_info("BLE deinitialized and memory reclaimed");
...
{
    debug_info("Identification callback");
    return ESP_OK;
}
...
extern "C" esp_err_t matter_light_init(void)
{
    esp_err_t err = ESP_OK;

    /* Initialize the ESP NVS layer */
    // nvs_flash_init(); // Do not need to initialize NVS here as it is already initialized in app_main.cpp before invoking this function. If needed, we can add a check here to see if NVS is already initialized or not.
    ...
    if (node == nullptr) {
        debug_info("Failed to create Matter node");
        return ESP_FAIL;
    }
    ...
    if (endpoint == nullptr) {
        debug_info("Failed to create extended color light endpoint");
        return ESP_FAIL;
    }
    ...
    {
        char debug_buf[64];
        sprintf(debug_buf, "Light created with endpoint_id %d", light_endpoint_id);
        debug_info(debug_buf);
    }
    ...
    if (endpoint == nullptr) {
        debug_info("Failed to create secondary network interface endpoint");
        return ESP_FAIL;
    }
#endif
    ...
    if (err != ESP_OK) {
        char debug_buf[64];
        sprintf(debug_buf, "Failed to start Matter, err:%d", err);
        debug_info(debug_buf);
        return err;
    }
    ...
    if (err != ESP_OK) {
        char debug_buf[64];
        sprintf(debug_buf, "Failed to initialized the encrypted OTA, err: %d", err);
        debug_info(debug_buf);
        return err;
    }
#endif // CONFIG_ENABLE_ENCRYPTED_OTA
